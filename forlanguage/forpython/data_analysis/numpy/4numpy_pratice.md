> copy from for learn: https://numpy123.com/article/advanced/numpy_exercises_for_data_analysis/
...
问题：在iris_2d数据集中的20个随机位置插入`np.nan`值
...
问题：将 iris_2d 的花瓣长度（第3列）加入以形成文本数组，这样如果花瓣长度为：
```python
Less than 3 --> 'small'
3-5 --> 'medium'
'>=5 --> 'large'
...
问题：在 iris_2d 中为卷创建一个新列，其中volume是`(np.pi * petallength * sepal_length^2) / 3`
