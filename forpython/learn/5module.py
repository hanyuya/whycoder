...
# 4、之所以我们说，正常的函数和变量名是公开的，是因为Python本身没有任何机制阻止我们直接访问private函数或变量，但是，按照约定俗成的规定，当函数或变量名以单下划线开头时，表示它是一个非公开的函数或变量，不应该被直接引用。
# 5、外部不需要引用的函数全部定义成private，只有外部需要引用的函数才定义为public。
...
# 举例：
# 1、安装requests模块，可以直接在命令行通过pip安装：
# pip install requests
# 2、安装lxml模块，可以先下载源码包，然后手动安装：
# 下载lxml源码包：
# wget https://pypi.python.org/packages/source/l/lxml/lxml-3.4.2.tar.gz
# 解压源码包：
# tar -zxvf lxml-3.4.2.tar.gz
# 进入源码包目录：
# cd lxml-3.4.2
# 安装lxml：
